...
x_obs6 = [0, 3,  3, 0, 0]; 
y_obs6 = [8, 8, 5, 5, 8];
plot( x_obs6, y_obs6, 'b')
fill( x_obs6, y_obs6, 'b')
hold on
    ...
    if (i > 1) %takes care of all other
      Collision = true;
        ...
        while (Collision == true) %loop that iterates if a new node point is in an obstacle
            ...
            if (inpolygon(newPoint(1,1),newPoint(1,2),x_obs,y_obs) == false && inpolygon(newPoint(1,1),newPoint(1,2),x_obs2,y_obs2) == false && inpolygon(newPoint(1,1),newPoint(1,2),x_obs3,y_obs3) == false && inpolygon(newPoint(1,1),newPoint(1,2),x_obs4,y_obs4) == false && inpolygon(newPoint(1,1),newPoint(1,2),x_obs5,y_obs5) == false && inpolygon(newPoint(1,1),newPoint(1,2),x_obs6,y_obs6)==false) %conditional statement that checks if a newly created node is inside an obstacle
                Collision = false;
