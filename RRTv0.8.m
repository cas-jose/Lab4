...
lineSize = 1; %The length of the line connecting nodes
...
x_obs_kal = [0, 0, 21, 21, 24, 24, 0];
y_obs_kal = [18, 11, 11, 0, 0, 18, 18];

x_obs_kal2 = [0, 0, 16, 16, 0];
y_obs_kal2 = [0, 8, 8, 0, 0];
    ...
    if (i > 1) %takes care of all other
      Collision = true;
        ...
        while (Collision == true) %loop that iterates if a new node point is in an obstacle
            ...
            if (inpolygon(newPoint(1,1),newPoint(1,2),x_obs,y_obs) == false...
                    && inpolygon(newPoint(1,1),newPoint(1,2),x_obs2,y_obs2) == false...
                    && inpolygon(newPoint(1,1),newPoint(1,2),x_obs3,y_obs3) == false...
                    && inpolygon(newPoint(1,1),newPoint(1,2),x_obs4,y_obs4) == false...
                    && inpolygon(newPoint(1,1),newPoint(1,2),x_obs5,y_obs5) == false...
                    && inpolygon(newPoint(1,1),newPoint(1,2),x_obs6,y_obs6) == false...
                    && inpolygon(newPoint(1,1),newPoint(1,2),x_obs_B,y_obs_B) == false...
                    && inpolygon(newPoint(1,1),newPoint(1,2),x_obs2_B,y_obs2_B) == false...
                    && inpolygon(newPoint(1,1),newPoint(1,2),x_obs3_B,y_obs3_B) == false...
                    && inpolygon(newPoint(1,1),newPoint(1,2),x_obs4_B,y_obs4_B) == false...
                    && inpolygon(newPoint(1,1),newPoint(1,2),x_obs5_B,y_obs5_B) == false...
                    && inpolygon(newPoint(1,1),newPoint(1,2),x_obs6_B,y_obs6_B) == false...
                    && inpolygon(newPoint(1,1),newPoint(1,2),x_obs_kal,y_obs_kal) == false...
                    && inpolygon(newPoint(1,1),newPoint(1,2),x_obs_kal2,y_obs_kal2) == false) %conditional statement that checks if a newly created node is inside an obstacle
                Collision = false;
...
theta = getAngles(path1, pointHistory);
fileID = fopen('TurnAngles.txt','wt');
fprintf(fileID, '%f\n', theta);
fclose(fileID);